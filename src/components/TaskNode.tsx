...
export type TaskNodeData = { task: Task; critical: boolean; highlighted: boolean; locked: boolean };
export type TaskFlowNode = Node<TaskNodeData, "task">;

export function TaskNode({ data, selected }: NodeProps<TaskFlowNode>) {
  const { task, critical, highlighted, locked } = data;
  const StatusIcon = task.kind === "milestone" ? Flag : task.status === "done" ? Check : task.status === "blocked" ? LockKeyhole : Play;
  return (
    <div className={`task-node status-${task.status} ${task.kind === "milestone" ? "milestone-node" : ""} ${critical ? "critical-node" : ""} ${locked ? "locked-node" : ""} ${highlighted ? "agent-highlight" : ""} ${selected ? "selected-node" : ""}`} data-testid={`task-node-${task.id}`}>
      ...
      <strong>{task.title}</strong>{locked && <span className="node-lock"><LockKeyhole size={9} /> Protected by human</span>}
