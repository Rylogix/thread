import { Braces, ChevronDown, Copy, GitCompareArrows, Menu, PanelLeft, PanelRight, PlayCircle, RefreshCw, Route, Sparkles } from "lucide-react";
    ...
    <main className="workspace-shell">
      <nav className="workspace-nav" aria-label="Workspace navigation">
        ...
        <div className="nav-actions">
          <button className="agent-pill" title={agentSupported ? `${registeredCount} tools registered` : "Open in a WebMCP-capable browser; manual planning remains available."}><span className={`status-dot ${agentSupported ? "online" : "offline"}`} /><b>{agentSupported ? "Agent connected" : "Agent unavailable"}</b><small>{agentSupported ? `${registeredCount} tools` : "38 tools ready"}</small></button>
